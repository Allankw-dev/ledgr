...
CREATE POLICY tenant_isolation ON schools FOR ALL TO ledgr_app
  USING (id = current_setting('app.current_school_id', true)::text)
  WITH CHECK (id = current_setting('app.current_school_id', true)::text);

DROP POLICY IF EXISTS tenant_isolation ON users;
CREATE POLICY tenant_isolation ON users FOR ALL TO ledgr_app
  USING (school_id = current_setting('app.current_school_id', true)::text)
  WITH CHECK (school_id = current_setting('app.current_school_id', true)::text);
...
CREATE POLICY tenant_isolation ON invoice_items FOR ALL TO ledgr_app
  USING (EXISTS (
    ...
    WHERE invoices.id = invoice_items.invoice_id
      AND invoices.school_id = current_setting('app.current_school_id', true)::text
  ))
  WITH CHECK (EXISTS (
    SELECT 1 FROM invoices
    WHERE invoices.id = invoice_items.invoice_id
...
CREATE POLICY tenant_isolation ON payment_plan_installments FOR ALL TO ledgr_app
  USING (EXISTS (
    ...
    WHERE payment_plans.id = payment_plan_installments.plan_id
      AND payment_plans.school_id = current_setting('app.current_school_id', true)::text
  ))
  WITH CHECK (EXISTS (
    SELECT 1 FROM payment_plans
    WHERE payment_plans.id = payment_plan_installments.plan_id
...
CREATE POLICY tenant_isolation ON student_guardians FOR ALL TO ledgr_app
  USING (EXISTS (
    ...
    WHERE students.id = student_guardians.student_id
      AND students.school_id = current_setting('app.current_school_id', true)::text
  ))
  WITH CHECK (EXISTS (
    SELECT 1 FROM students
    WHERE students.id = student_guardians.student_id
...
CREATE POLICY tenant_isolation ON mpesa_transactions FOR ALL TO ledgr_app
  USING (school_id IS NULL OR school_id = current_setting('app.current_school_id', true)::text)
  WITH CHECK (school_id IS NULL OR school_id = current_setting('app.current_school_id', true)::text);
