    ...
    items = [
        InvoiceListItem(
            **InvoiceResponse.model_validate(invoice).model_dump(),
            ...
            has_active_payment_plan=invoice.id in active_plan_invoice_ids,
        )
...
def update_invoice(
    ...
    """Corrects a wrong due date after the fact — e.g. it was generated
    with the wrong term's deadline. Nothing money-related is editable here;
    see /void for invoices that need to be undone entirely."""
    invoice = db.execute(
    ...
    if invoice.status == InvoiceStatus.CANCELLED:
        raise HTTPException(409, "This invoice has been voided and can no longer be edited")
...
def void_invoice(
    ...
    """Cancels an invoice that was generated by mistake — wrong student,
    wrong term, duplicate run of bulk-generate, etc. Only allowed while
    amount_paid is still zero: once real money has been recorded against
    an invoice, voiding it would silently orphan that payment history, so
    that case needs a manual correction instead of a one-click undo."""
    invoice = db.execute(
        select(Invoice).where(Invoice.id == invoice_id, Invoice.school_id == school_id)
    ).scalar_one_or_none()
    if not invoice:
    ...
    if invoice.amount_paid > 0:
        raise HTTPException(409, "Can't void an invoice with payments already recorded against it")
    if invoice.status == InvoiceStatus.CANCELLED:
        raise HTTPException(409, "This invoice is already voided")
